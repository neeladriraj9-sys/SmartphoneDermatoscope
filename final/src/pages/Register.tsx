...
const Register = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    const parsed = schema.safeParse({ name, email, mobile });
    if (!parsed.success) {
  ...
  return (
    <AuthLayout title="Create your account" subtitle="Start checking your skin in under a minute.">
      <Seo title="Create Account" description="Sign up for SkinScan AI." />
      <form onSubmit={onSubmit} className="space-y-4" noValidate>
        ...
        <p className="text-sm text-center">
          Already have an account? <Link to="/login" className="text-primary-dark hover:underline font-semibold">Log in</Link>
        </p>
      </form>
    </AuthLayout>
  );
